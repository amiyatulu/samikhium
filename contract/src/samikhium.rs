...
pub mod samikhiumstructs;
pub use self::samikhiumstructs::{CommentProduct, CommentReview, Product, Review, User};
...
impl Samikhium {
    ...
    pub fn add_product_crowdfunding(&mut self, bounty:U128, product_id:U128) {
        ...
        assert!(account_id != product_user_id_name, "You can't fund your won product");
        ...
        match product_bounty_crowdfunding_exists_option {
            Some(bountyvalue) => {
                ...
                self.product_crowdfunding.insert(&product_id, &new_bounty_value);
            }
            None => {
                self.burn(&account_id, bounty);
                self.mint(&product_user_id_name, bounty);
...
near_contract_standards::impl_fungible_token_core!(Samikhium, ft);
near_contract_standards::impl_fungible_token_storage!(Samikhium, ft);

impl FungibleTokenMetadataProvider for Samikhium {
    fn ft_metadata(&self) -> FungibleTokenMetadata {
        FungibleTokenMetadata {
            spec: FT_METADATA_SPEC.to_string(),
            name: String::from("Samikhium fungible token"),
